...
| Background tile mip levels | When the viewport is zoomed out enough, the background tile system should fetch or generate a lower-resolution canonical mip rather than always use the full-resolution source. The system supports at least eight levels, each using `max(1, ceil(nativeDimension / 2^level))`, deterministic low-pass reduction for synthetic imagery, and a stable source-scoped identity plus content revision. Canvas and raster inputs remain source-agnostic: a materializer/cache resolves descriptor-plus-request contracts asynchronously, while the synchronous rasterizer receives only a validated resident payload or placeholder. | ICW-076, ADR-0005 | Cache keys, reservations, and visible-frame pins include source ID, tile ID, content revision, and mip level; accounting uses actual payload bytes. Pixelometer reads use an explicit non-blocking mip-zero path until a source-neutral point-sampling contract exists. |
